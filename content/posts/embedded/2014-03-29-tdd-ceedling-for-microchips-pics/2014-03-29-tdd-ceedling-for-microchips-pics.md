...
- на уровне периферии код часто так и остаётся недопокрытым тестами. К примеру периферийные модули имеют свойство по характерным событиям выбрасывать флаги, наполнять регистры данными - изобретать велосипеды и придумывать фейковые периферийные модули ? Как минимум нужно прекрасно пронимать как эта периферия работает, плюс асинхронность

- ну а о тестировании фич спецефичных для архитектуры микроконтроллера типа [MIPS16](http://www.microchip.com/stellent/groups/SiteComm_sg/documents/DeviceDoc/en557154.pdf){:rel="nofollow"} вообще и говорить не приходится
