title: Паттерны в чистом C. State
category: Embedded
...
В **С** конечно нету классов, но пока для простоты можно считать что класс и модуль (файл) в нашем примере одно и тоже. Теперь попробуем набросать [UML](http://ru.wikipedia.org/wiki/UML){:rel="nofollow"} - диаграмму для нашей гирлянды (если вы не очень сильны в *UML*, не волнуйтесь - я тоже):
