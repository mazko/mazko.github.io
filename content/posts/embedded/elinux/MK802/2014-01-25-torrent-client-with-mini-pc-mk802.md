...
В MK802 имеется один *USB-хост* разьём, со временем туда захочется вставить что-то полезное вроде флешки. Или хаб, а в него уже нескольео флешек. С помощью [LVM2](http://ru.wikipedia.org/wiki/LVM){:rel="nofollow"} можно склеить диски в один. Понадобится специальный модуль ядра ```dm-mod```:
